...
vx_baseline = np.diff(global_x_baseline)/0.15
vx_baseline = np.hstack((vx_baseline,vx_baseline[-1]))
vy_baseline = np.diff(global_y_baseline)/0.15
vy_baseline = np.hstack((vy_baseline,vy_baseline[-1]))
...
for jj in range(0,num,3):
    ...
    patch_ego_global_current = pt.Ellipse((global_x_baseline[jj],global_y_baseline[jj]),width=6.0,height=3.0,angle=np.rad2deg(psi_global_baseline[jj]),\
                                            facecolor='blue',edgecolor='black',alpha=alpha)

    ax.add_patch(patch_obs_global_current)
    ax.add_patch(patch_ego_global_current)

    ax.plot(global_x_baseline[jj:],global_y_baseline[jj:] ,color='b',linewidth=4.0,label="Ego trajectory")
    ...
    for ii in range(0,len(x_obs_r_single_baseline)):
        ax.plot(x_obs_r_single_baseline[ii,:],y_obs_r_single_baseline[ii,:],color="r",linewidth=6*linewidth,alpha=1)
    for ii in range(0,len(x_obs_up_val)):
        ax.plot(x_obs_up_val[ii,:],y_obs_up_val[ii,:],color="k",linewidth=0.1*linewidth,alpha=0.8)   

    ax.text(text_x,20,"Ego velocity: {}".format(np.sqrt(vy_baseline[jj]**2+vx_baseline[jj]**2)))

    if(jj>=11):
        ax.text(0,20,"Reduced set [6] computed based on geometric conflict \n with initial trajectory, chooses the less probable samples \
                \n and slows down ")
...
animation.save('videos/animation_baseline.mp4')
